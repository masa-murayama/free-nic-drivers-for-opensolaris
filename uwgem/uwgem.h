...
#define		LW_SCAN_TIMEOUT		drv_usectohz(200*1000)	/* scan time for a channnel */
#else
#define		LW_SCAN_TIMEOUT		drv_usectohz(10000*1000)	/* for total scan time */
#endif
...
#ifdef CONFIG_NEW_SCAN
	ksema_t			lw_scan_lock;	/* serialize scan */
	boolean_t		lw_scan_running;
#else
	int			lw_scan_count;
#endif
...
#define		RXMODE_BITS	\
	...
	struct uwgem_conf {
		...
		/* crypto operations */
		int (*uwgc_key_alloc)(struct ieee80211com *,
		    const struct ieee80211_key *,
		    ieee80211_keyix *, ieee80211_keyix *);
		int (*uwgc_key_delete)(struct ieee80211com *,
		    const struct ieee80211_key *);
		int (*uwgc_key_set)(struct ieee80211com *,
		    const struct ieee80211_key *,
		    const uint8_t mac[IEEE80211_ADDR_LEN]);
		void (*uwgc_key_update_begin)(struct ieee80211com *);
		void (*uwgc_key_update_end)(struct ieee80211com *);
	} ugc;
...
#define UWGEM_VLAN	0x0001
	...
	/* serializer for crypto */
	ksema_t			crypto_tx;
	ksema_t			crypto_rx;

	/* hw scan support */
	boolean_t	hw_scan_done;
};
...
int uwgem_bulk_out(struct uwgem_dev *dp, mblk_t *mp, int qid, int async, int timeout, int retry);
