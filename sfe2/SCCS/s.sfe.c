h24156
s 00058/00039/03896
d D 1.5 02/09/09 23:38:28 mrym 5 4
c 0.8.6
e
s 00169/00251/03766
d D 1.4 02/08/25 21:03:47 mrym 4 3
c 0.8.5
e
s 00013/00007/04004
d D 1.3 02/08/22 19:32:47 mrym 3 2
c release 0.8.3
e
s 01121/00483/02890
d D 1.2 02/08/01 01:46:30 mrym 2 1
c 0.8.2
e
s 03373/00000/00000
d D 1.1 02/04/20 00:03:10 mrym 1 0
c date and time created 02/04/20 00:03:10 by mrym
e
u
U
f e 0
...
D 2
 *  of the GNU Lesser Public License, incorporated herein by reference.
E 2
I 2
...
/*
 TO DO:
 802.3x flow contorol support
	sis900 TX pause on reception of pause packet - OK
	       send pause packet - ?
	83815 TX pause on reception of pause packet - NO
	       send pause packet - ?
RX flow control does not work.
 Clearfy MII resetting policy and suppress warning messages
 */
E 2
...
D 2
char	ident[] = "sfe v" VERSION "test1";
E 2
I 2
...
D 2
/* I/O instructions */
#define	OUTB(dp, p, v)		outb((int)((dp)->base_addr + (p)), v)
#define	OUTW(dp, p, v)		outw((int)((dp)->base_addr + (p)), v)
#define	OUTL(dp, p, v)		outl((int)((dp)->base_addr + (p)), v)
#define	INB(dp, p)		inb(((int)(dp)->base_addr + (p)))
#define	INW(dp, p)		inw(((int)(dp)->base_addr + (p)))
#define	INL(dp, p)		inl(((int)(dp)->base_addr + (p)))
E 2
I 2
/*
 * I/O instructions
...
#define	DPRINTF(n, args...)	\
	if (sfe_debug > (n)) cmn_err(CE_CONT, args)
E 2
I 2
#define	DPRINTF(n, args)	\
...
D 4
#define TX_TIMEOUT		(drv_usectohz(10*1000000))
E 4
I 4
...
D 2
	void 			(*get_mac_addr)(struct sfe_dev *);
E 2
I 2
...
D 2
static int sfe_chipinfo_init_dp83815(struct sfe_dev *dp);
static int sfe_chipinfo_init_sis900(struct sfe_dev *dp);
E 2
I 2
static boolean_t sfe_chipinfo_init_dp83815(struct sfe_dev *dp);
static boolean_t sfe_chipinfo_init_sis900(struct sfe_dev *dp);
D 4
static boolean_t sfe_chipinfo_init_sis7016(struct sfe_dev *dp);
E 4
E 2
...
/* DDI/DKI functions */
D 5
static int sfeprobe(dev_info_t *);
E 5
...
D 4
#ifdef DEBUG_SIS7016
	0x1039,	0x0900,	"SiS7016debug", CHIPTYPE_SIS7016, sfe_chipinfo_init_sis7016,
#else
E 4
E 2
...
I 2
 ...
 {
D 4
	0x1039,	0x7016,	"SiS7016", CHIPTYPE_SIS7016, sfe_chipinfo_init_sis7016,
E 4
I 4
...
E 2
	...
	if (cmd == DDI_ATTACH) {
D 2
		DPRINTF(2, "sfeattach: called cmd:ATTACH");
E 2
I 2
...
E 2
		 ...
		 */
D 2
		if (ddi_regs_map_setup(dip, 1, (caddr_t *)&dp->base_addr,
E 2
I 2
		/* Search IO-range */
		regs = NULL;
...
D 2
				"Unknown PCI venid/devid (0x%x, 0x%x)",
				dp->name, vid, did);
E 2
I 2
...
D 2
		cmn_err(CE_CONT, "?%s: nic chip is %s",
...
D 2
			cmn_err(CE_WARN, "%s: %s: reset chip failed.",
...
D 2
		DPRINTF(3, "%s: reset done", dp->name);
E 2
I 2
...
D 2
		DPRINTF(2, "sfeattach: return: successed");
E 2
I 2
...
I 4
		...
		if (dp->buf_allocated != dp->buf_free) {
			 ...
			 */
D 5
			cmn_err(CE_NOTE, "!%s: sfedetach: some buffer are busy",
				dp->name);
E 5
I 5
			cmn_err(CE_NOTE,
...
x:
	...
	if (i > 4) {
		cmn_err(CE_CONT,
D 2
	"%s: %s: frags:%d (%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d)",
			dp->name, __FUNCTION__, i,
E 2
I 2
...
D 2
	DPRINTF(2, "%s: %s: tx-ing a packet (seqnum %d-%d) using slot %d-%d",
		dp->name, __FUNCTION__,
E 2
I 2
	DPRINTF(2, (CE_CONT,
...
sfe_receive(struct sfe_dev *dp)
{
	 ...
	 */
D 2
	DPRINTF(3, "%s: rx_head:%d, status:%b",
			__FUNCTION__,
			dp->rx_head, 
E 2
I 2
...
D 2
	DPRINTF(3, "%s: %s: called: isr:0x%b",
		dp->name, __FUNCTION__, isr, INTR_BITS);
E 2
I 2
...
D 4
	if (dp->chip->chip_type == CHIPTYPE_SIS900 ||
	    dp->chip->chip_type == CHIPTYPE_SIS7016) {
E 4
I 4
	if (dp->chip->chip_type == CHIPTYPE_SIS900) {
E 4
...
#ifdef notdef
D 2
	DPRINTF(3, "%s: %s: return: isr: %b",
	       dp->name, __FUNCTION__, INL(dp, ISR), INTR_BITS);
E 2
I 2
...
D 2
	DPRINTF(4, "%s: %s: called", dp->name, __FUNCTION__);
E 2
I 2
...
E 2
	...
	OUTL(dp, RFCR, (RFADDR_MAC_SIS900 + 0) << RFCR_RFADDR_SHIFT_SIS900);
	OUTL(dp, RFDR, (mac[1] << 8) | mac[0]);
	OUTL(dp, RFCR, (RFADDR_MAC_SIS900 + 1) << RFCR_RFADDR_SHIFT_SIS900);
	OUTL(dp, RFDR, (mac[3] << 8) | mac[2]);
	OUTL(dp, RFCR, (RFADDR_MAC_SIS900 + 2) << RFCR_RFADDR_SHIFT_SIS900);
	OUTL(dp, RFDR, (mac[5] << 8) | mac[4]);
I 2
#ifdef notdef
	OUTL(dp, RFCR, (RFADDR_MAC_SIS900 + 0) << RFCR_RFADDR_SHIFT_SIS900);
	m[0] = INL(dp, RFDR);
	...
	OUTL(dp, RFCR, (RFADDR_MAC_SIS900 + 2) << RFCR_RFADDR_SHIFT_SIS900);
	m[2] = INL(dp, RFDR);
	DPRINTF(0, (CE_CONT, "%s: %04x %04x %04x", dp->name, m[0], m[1], m[2]));
#endif
E 2
...
D 5
	DPRINTF(0, (CE_CONT, "%s: sfe_get_mac_addr_sis635: called", dp));
E 5
I 5
...
static void
D 4
sfe_mii_sync_sis7016(struct sfe_dev *dp)
E 4
I 4
...
I 2
D 4
sfe_mii_read_sis7016(struct sfe_dev *dp, int reg)
E 4
I 4
...
static void
D 4
sfe_mii_write_sis7016(struct sfe_dev *dp, int reg, uint16_t val)
E 4
I 4
...
static uint16_t
E 2
sfe_mii_read_sis900(struct sfe_dev *dp, int offset)
{
	uint32_t	val;
	int		i;

	val = (offset << ENPHY_OFFSET_SHIFT) | ENPHY_RDCMD;

	OUTL(dp, ENPHY, val | ENPHY_ACCESS);
	drv_usecwait(10);

	i = 100;
	while (i--) {
		if (((val = INL(dp, ENPHY)) & (ENPHY_ACCESS)) == 0) {
			/* done */
D 2
			DPRINTF(4, "%s: %s: delay:%d val:0x%x",
				dp->name, __FUNCTION__, 100-i, val);
E 2
I 2
			DPRINTF(4, (CE_CONT,
				"%s: sfe_mii_read_sis900: delay:%d val:0x%x",
				dp->name, 100-i, val));
E 2
			return val >> ENPHY_DATA_SHIFT;
		}
		drv_usecwait(10);
	}
D 2
	cmn_err(CE_WARN, "%s: %s: timeout", __FUNCTION__);
E 2
I 2
	cmn_err(CE_WARN, "%s: sfe_mii_read_sis900: timeout", dp->name);
E 2
	return -1;
}

static void
sfe_mii_write_sis900(struct sfe_dev *dp, int offset, uint16_t val)
{
	int		i;
	uint32_t	tmp;

D 2
	tmp = (((uint32_t)val) << ENPHY_DATA_SHIFT) |
	       (offset << ENPHY_OFFSET_SHIFT);

E 2
I 2
	tmp = (val << ENPHY_DATA_SHIFT) | (offset << ENPHY_OFFSET_SHIFT);
E 2
	OUTL(dp, ENPHY, tmp | ENPHY_ACCESS);
	drv_usecwait(10);

	i = 100;
	while (i--) {
		if ((INL(dp, ENPHY) & (ENPHY_ACCESS)) == 0) {
			/* done */
			return ;
		}
		drv_usecwait(10);
...
E 2
I 2
	cmn_err(CE_WARN, "%s: sfe_mii_write_sis900: timeout", dp->name);
E 2
	return;
}
...
D 4
static boolean_t
sfe_chipinfo_init_sis7016(struct sfe_dev *dp)
{
	dp->get_mac_addr  = &sfe_get_mac_addr_sis900;
	dp->mii_write_reg = &sfe_mii_write_sis7016;
	dp->mii_read_reg  = &sfe_mii_read_sis7016;
	dp->set_rx_filter = &sfe_set_rx_filter_sis900;
...
D 2
	DPRINTF(1, "?%s: cfg:%b", dp->name, INL(dp, CFG), CFG_BITS);
E 2
I 2
...
D 2
#define	SFE_EEPROM_DELAY(dp)	drv_usecwait(1)
E 2
I 2
...
E 2
	SFE_EEPROM_DELAY(dp);
D 2
	OUTL(dp, EROMAR, val | EROMAR_EESK);
E 2
I 2
...
D 2
#ifdef notdef
	cmn_err(CE_CONT, "%s: %s: %s duplex, %d Mbps",
		dp->name, __FUNCTION__,
...
D 2
		DPRINTF(1, "?%s: PCR: %b", dp->name, INL(dp, PCR), PCR_BITS);
	}
	else if (dp->chip->chip_type == CHIPTYPE_SIS900) {
		if (dp->flow_control) {
...
sfe_mii_config(struct sfe_dev *dp)
{
	...
	DPRINTF(2, "%s: %s: called", dp->name, __FUNCTION__);
	...
	DPRINTF(2, "%s: %s: MII_STATUS reg:%b",
		dp->name, __FUNCTION__, mii_stat, MII_STATUS_BITS);
...
#endif
	DPRINTF(2, "%s: %s: setting MII_AN_ADVERT reg:%b",
		dp->name, __FUNCTION__, val, MII_ABILITY_BITS);
...
sfe_mii_link_watcher(struct sfe_dev *dp)
{
	...
	/* read PHY status */
	mii_stat = mii_read(dp, MII_STATUS);
	DPRINTF(4, "%s: %s: called: mii_state:%d MII_STATUS reg:%b",
		dp->name, __FUNCTION__, dp->mii_state,
	...
	if (dp->mii_state != MII_STATE_LINKDOWN &&
		...
		DPRINTF(1, "?%s: link up detected: mii_stat:%b",
			dp->name, mii_stat, MII_STATUS_BITS);
		...
		if (!dp->mii_fixedmode) {
			...
			DPRINTF(1, "?%s: %s: advert:%b, lpable:%b",
				dp->name, __FUNCTION__,	
...
D 4
	...
	if (dp->mii_state = MII_STATE_LINKUP) {
		...
		if (rev== SIS630A_900_REV && 
			...
			mii_write(dp, MII_RESV, (reg14h | 0x2200) & 0xBFFF);
		}
		else {
			mii_write(dp, MII_RESV, (reg14h | 0x2000) & 0xBFFF);
		}
	}
#endif /* TUNE_PHY_630 */
	return;
...
sfe_mii_config(struct sfe_dev *dp)
{
	...
	if (dp->chip->chip_type == CHIPTYPE_SIS7016) {
E 4
I 4
	if (dp->chip->chip_type == CHIPTYPE_SIS900) {
E 4
...
D 4
		if ((dp->mii_phyid & 0xfffffff0) == 0x0015F440) {
E 4
I 4
...
sfe_mii_link_watcher(struct sfe_dev *dp)
{
	...
	if (dp->mii_state != MII_STATE_LINKDOWN &&
		...
		if (dp->chip->chip_type == CHIPTYPE_SIS900) {
D 4
			if ((dp->mii_phyid & 0xfffffff0) == 0x001D8000) {
E 4
I 4
