...
/*
	Written/copyright 1994-2004 by Donald Becker.
...
static struct pci_id_info pci_id_tbl[] = {
	...
	{ "3Com 3cSOHO100B-TX (ADMtek Centaur)", { 0x930010b7, 0xffffffff },
	  TULIP_IOTYPE, TULIP_SIZE1, COMET },
	{ "3Com 3cSOHO100B-TX (ADMtek Centaur, 8c00)", { 0x8c0010b7, 0xffffffff },
	  TULIP_IOTYPE, TULIP_SIZE1, COMET },
	{ 0},
};
...
static int read_eeprom(struct net_device *dev, int location, int addr_len);
						  ...
						  long ioaddr, int irq, int pci_tbl_idx, int find_cnt)
{
	...
	if (chip_idx == DC21040) {
		...
		int sa_offset = 0;
		int ee_addr_size = read_eeprom(dev, 0xff, 8) & 0x40000 ? 8 : 6;
		int eeprom_word_cnt = 1 << ee_addr_size;

		for (i = 0; i < eeprom_word_cnt; i++)
			((u16 *)ee_data)[i] =
				le16_to_cpu(read_eeprom(dev, i, ee_addr_size));
	...
	tp->pci_dev = pdev;
	tp->msg_level = debug > 31 ? debug : (1 << debug) - 1;
	tp->chip_id = chip_idx;
...
/* Delay between EEPROM clock transitions.
   ...
   udelay() unreliable.
*/
#define eeprom_delay(ee_addr)  do {	inl(ee_addr); } while(0)
...
static int read_eeprom(struct net_device *dev, int location, int addr_len)
{
	int i;
	unsigned retval = 0;
	long ee_addr = dev->base_addr + CSR9;
