...
static void sky2_phy_init(struct sky2_hw *hw, unsigned port)
{
	...
	if (sky2->autoneg == AUTONEG_ENABLE &&
	    !(hw->flags & SKY2_HW_NEWER_PHY)) {	/* done */
	...
	if (sky2_is_copper(hw)) {
		if (!(hw->flags & SKY2_HW_GIGABIT)) {
			...
			if (hw->chip_id == CHIP_ID_YUKON_FE_P && /* done */
			...
			if (sky2->autoneg == AUTONEG_ENABLE	/* done */
	...
	} else if (hw->chip_id == CHIP_ID_YUKON_FE_P &&		/* done */
		   hw->chip_rev == CHIP_REV_YU_FE2_A0) {
		/* apply workaround for integrated resistors calibration */
		gm_phy_write(hw, port, PHY_MARV_PAGE_ADDR, 17);
		gm_phy_write(hw, port, PHY_MARV_PAGE_DATA, 0x3f60);
	} else if (hw->chip_id != CHIP_ID_YUKON_EX) {	/* done */
...
static void sky2_mac_init(struct sky2_hw *hw, unsigned port)
{
	...
	sky2_write8(hw, SK_REG(port, RX_GMF_CTRL_T), GMF_RST_CLR); /* done */
				    ...
				    u16 length, u32 status)
{
	 ...
	 */
	if (sky2->hw->chip_id == CHIP_ID_YUKON_FE_P &&	/* done */
