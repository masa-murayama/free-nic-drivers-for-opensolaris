...
#ifndef MODULE
int mtd80x_probe(struct net_device *dev)
{
        if (pci_drv_register(&mtd80x_drv_id, dev) < 0)
                return -ENODEV;
...
int init_module(void)
{
        if (debug)      /* Emit version even if no cards detected. */
                printk(KERN_INFO "%s" KERN_INFO "%s", version1, version2);
        return pci_drv_register(&mtd80x_drv_id, NULL);
}
...
void cleanup_module(void)
{
        ...
        pci_drv_unregister(&mtd80x_drv_id);
