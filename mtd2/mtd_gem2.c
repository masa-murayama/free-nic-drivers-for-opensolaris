/* mtd80x.c: A Linux device driver for the mtd80x Ethernet chip. */
/*
...
"mtd80x.c:v0.09 7/26/2001 Written by Donald Becker <becker@scyld.com>\n";
static const char version2[] =
"  http://www.scyld.com/network/drivers.html\n";
static const char version3[] =
"mtd80x.c:v0.21 8/27/2001 Modified\n";
...
MODULE_DESCRIPTION("Digitalchina EDA-100 Fast Ethernet Card driver");
...
static void *mtd80x_probe1(struct pci_dev *pdev, void *init_dev,
...
struct drv_id_info mtd80x_drv_id = {
        "mtd80x", 0, PCI_CLASS_NETWORK_ETHERNET<<8, pci_id_tbl, mtd80x_probe1,
        netdev_pwr_event };
...
static int
mdio_read(struct net_device *dev, int phy_id, unsigned int location)
{
...
static void
resetnic(struct net_device *dev)
{
        /* Stop Tx and Rx */
	OUTL(RxConfig, 0);
        for (i = 0; i < 200; i++) {
		if ((INL(RxConfig) & (TxEnable | RxEnable)) == 0) {
			/* done */
			break;
		}
		drv_usecwait(10);
...
mtd_init_chip()
{
	...
	OUTL(dp, RxRingPtr);
        ...
        /* Configure the PCI bus bursts and FIFO thresholds. */
        OUTL(dp, PCIBusCfg, 0x38);

	/* Enable interrupts by setting the interrupt mask. */
	OUTL(dp, IntrEnable, (IntrRxDone | IntrRxErr | IntrRxEmpty | IntrTxDone | IntrTxEmpty
                   | StatsMax | RxOverflow | TxUnderrun | IntrPCIErr | NWayDone
                   | LinkChange);

	OUTL(dp, RxConfig, np->txrx_config);
	OUTL(dp, TxStartDemand, 0);
	OUTL(dp, RxStartDemand, 0);
}

static int
mtd_tx_desc_write(struct sk_buff *skb, struct net_device *dev)
{
        tdp->buf_addr = LE32(dmacookie->dmac_address);
        tdp->ctrl_length =
                LE32(TxIntrOnDone | TxNormalPkt | (dmacookie->dmac_size << 11) | dmacookie->dmac_size);
        tdp->status = LE32(DescOwn);
...
/* The interrupt handler does all of the Rx thread work and cleans up
   after the Tx thread. */
static void
intr_handler(int irq, void *dev_instance, struct pt_regs *rgs)
{
	intr_status = INL(dp, IntrStatus);

	/* Acknowledge all of the current interrupt sources ASAP. */
	OUTL(dp, IntrStatus, intr_status);
...
static int
mtd_close(struct net_device *dev)
{
