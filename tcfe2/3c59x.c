				   ...
				   int chip_idx, int card_idx)
{
	...
	EL3WINDOW(0);
	{
		...
		for (i = 0; i < 0x40; i++) {
			int timer;
			iowrite16(base + i, ioaddr + Wn0EepromCmd); /* X */
	...
	EL3WINDOW(2);
	for (i = 0; i < 6; i++)
		iowrite8(dev->dev_addr[i], ioaddr + i); /* X */
...
issue_and_wait(struct net_device *dev, int cmd)
{
	...
	iowrite16(cmd, ioaddr + EL3_CMD); /* X */
	for (i = 0; i < 2000; i++) {
...
vortex_up(struct net_device *dev)
{
	...
	iowrite16(SetStatusEnb | 0x00, ioaddr + EL3_CMD); /* X */
...
/* XXX */
	...
	if (vp->full_bus_master_tx) { 		/* Boomerang bus master Tx. */
		...
		if (vp->drv_flags & IS_BOOMERANG)
			iowrite8(PKT_BUF_SZ>>8, ioaddr + TxFreeThreshold); /* Room for a packet. */ /* X */
...
static void vortex_tx_timeout(struct net_device *dev)
{
	...
	if (vp->full_bus_master_tx) {
		...
		if (vp->drv_flags & IS_BOOMERANG)
			iowrite8(PKT_BUF_SZ>>8, ioaddr + TxFreeThreshold); /* X */
...
boomerang_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	issue_and_wait(dev, DownStall);	/* X */
	...
	iowrite16(DownUnstall, ioaddr + EL3_CMD);	/* X */
...
boomerang_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	do {
		...
		if (status & UpComplete) {
			iowrite16(AckIntr | UpComplete, ioaddr + EL3_CMD); /* X */
			if (vortex_debug > 5)
		...
		if (status & DownComplete) {
			...
			iowrite16(AckIntr | DownComplete, ioaddr + EL3_CMD); /* X */
...
boomerang_rx(struct net_device *dev)
{
	...
	for (; vp->cur_rx - vp->dirty_rx > 0; vp->dirty_rx++) {
		...
		iowrite16(UpUnstall, ioaddr + EL3_CMD); /* X */
	}
	return 0;
}
...
static void set_8021q_mode(struct net_device *dev, int enable)
{
	...
	if ((vp->drv_flags&IS_CYCLONE) || (vp->drv_flags&IS_TORNADO)) {
		...
		EL3WINDOW(3);
		iowrite16(max_pkt_size, ioaddr+Wn3_MaxPktSize); /* X */

		/* set VlanEtherType to let the hardware checksumming
		   treat tagged frames correctly */
		EL3WINDOW(7);
		iowrite16(VLAN_ETHER_TYPE, ioaddr+Wn7_VlanEtherType); /* X */
	} else {
