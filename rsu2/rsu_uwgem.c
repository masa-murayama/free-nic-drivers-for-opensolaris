...
char	ident[] = "RTL8188SU driver v" VERSION;
...
rsu_fw_cmd(struct uwgem_dev *dp, uint8_t code, void *buf, int len)
{
	...
	return (uwgem_bulk_out(dp, mp, lp->qid2idx[RSU_QID_H2C], 0, 10, 1));
}
...
rsu_newstate(struct uwgem_dev *dp, enum ieee80211_state nstate, int arg)
{
	...
	ostate = ic->ic_state;
#ifdef DEBUG_LEVEL
	if (rsu_debug > 0 && !(ostate == nstate && nstate == IEEE80211_S_AUTH)) {
		cmn_err(CE_CONT,
	...
	case IEEE80211_S_SCAN:
		if (ostate != IEEE80211_S_SCAN) {
			...
			mutex_exit(&lp->scan_done_lock);
#else
			/* stop calling next_scan by lw_thread until hw scan has done */
			dp->hw_scan_done = B_FALSE;
#endif
	...
	case IEEE80211_S_RUN:
		...
		DPRINTF(0, (CE_CONT,
		    ...
		    in->in_reqcw,
		    in->in_chw, in->in_chan ? in->in_chan->ich_flags : 0,
		    in->in_htstbc));
		DPRINTF(1, (CE_CONT,
...
rsu_join_bss(struct uwgem_dev *dp, struct ieee80211_node *ni)
{
	...
	if ((ic->ic_flags_ext & IEEE80211_FEXT_HT) &&
	    ni->in_htcap_ie != NULL) {
		struct ieee80211_ie_htcap *htcapp;
#if 1	/* debug */
		uwgem_dump_ie(ni->in_htcap_ie);
#endif
		if (ni->in_wme_ie && ni->in_wme_ie[1] >= 7) {
#if 1	/* debug */
			uwgem_dump_ie(ni->in_wme_ie);
#endif
			for (i = 0; i < 9; i++) {
				frm[i] = ni->in_wme_ie[i];
			}
			frm[1] = 7;
			frm[6] = 0;
			frm[8] = 0;
			...
			frm = ieee80211_add_wme_info(frm, NULL);
		}
		htcapp = (void *)frm;
		frm = ieee80211_add_htcap(frm, ni);
#ifdef notdef /* debug */
#if 0	/* debug */
		htcapp->hc_param = 7 << 2 | 0;	/* test */
		htcapp->hc_param = 0x3;	/* org */
#endif
		/* clear unused part */
		bzero(htcapp->hc_mcsset, sizeof (htcapp->hc_mcsset));	/* org */

		htcapp->hc_extcap = 0;
		htcapp->hc_txbf = 0;
		htcapp->hc_antenna = 0;
#endif /* notdef */
		DPRINTF(0, (CE_CONT, "!%s: %s join w/ htcap:%04x htparam:%02x",
		    dp->name, __func__, htcapp->hc_cap, htcapp->hc_param));
	}
#endif
	if (ic->ic_opt_ie != NULL) {
		/* WPA requrired option ie */
...
rsu_event_join_bss(struct uwgem_dev *dp, uint8_t *buf, int len)
{
	...
	uwgem_dump_pkt(__func__, (void *)&rsp[1], letoh32(rsp->bss.ieslen), -1, rsp->bss.rssi);
#else
	/* 
	 * XXX - the ie join_bss event returns just includes wme and htcap like below.
	 * dd070050 f2020001 00  (wme)
	 * 2d1ae2 18030000 00000000 00000000 00000000 00000000 00000000 00 (htcap)
	 * we need rate and xrate to form an ASSOC_RESP frame solaris 80211 expects.
	 * It seems what we attached on join bss request !
	 */
/*
...
*/
{
	...
	uint8_t	*frm;
        ...
        /* Build a fake ASSOC_RESP frame to let net80211 do all the parsing. */
        pktlen = sizeof (*wh) + 2*3 + letoh32(rsp->bss.ieslen) - 12;
	pktlen = 1024;
	...
	/* make fixed part */
	frm[0] = ((uint8_t *)(&rsp[1]))[8 + 2];	/* capability info */
	frm[1] = ((uint8_t *)(&rsp[1]))[8 + 3];	/* capability info */
	...
	*frm++ = IEEE80211_ELEMID_RATES;
	n = min(ni->in_rates.ir_nrates, IEEE80211_RATE_SIZE);
	*frm++ = n;
	bcopy(ni->in_rates.ir_rates, frm, n);
	frm += n;

	/* add extended supperted rates */
	if (ni->in_rates.ir_nrates > IEEE80211_RATE_SIZE) {
		n = ni->in_rates.ir_nrates - IEEE80211_RATE_SIZE;

		*frm++ = IEEE80211_ELEMID_XRATES;
		*frm++ = n;
		bcopy(ni->in_rates.ir_rates + IEEE80211_RATE_SIZE, frm, n);
		frm += n;
	}
	...
	if (ni->in_wme_ie) {
		n = ni->in_wme_ie[1] + 2;
		bcopy(ni->in_wme_ie, frm, n);
		frm += n;
	}

	/* add ht cap */
	if (ni->in_htcap_ie) {
		n = ni->in_htcap_ie[1] + 2;
		bcopy(ni->in_htcap_ie, frm, n);
		frm += n;
	}
	...
	mp->b_cont = (void *)(uintptr_t)letoh32(rsp->bss.rssi);
	mp->b_prev = (void *)(uintptr_t)0;
...
*/
	...
	case R92S_EVT_FWDBG:
		buf[len-1] = '\0';
		DPRINTF(10, (CE_CONT, "!%s: %s: FWLOG: %s",
		    dp->name, __func__, (char *)buf));
		break;

	case R92S_EVT_SURVEY_TIMER:
		/* ignore */
		break;
    ...
    mblk_t *mp_head, mblk_t *mp)
{
	...
	uint32_t	rxdw0, rxdw1, rxdw2, rxdw3;
	uint8_t	rate;
	int8_t	rssi = 0;
	int	s, infosz;
	...
	rxdw1 = letoh32(stat->rxdw1);
	rxdw2 = letoh32(stat->rxdw2);
	rxdw3 = letoh32(stat->rxdw3);

	if ((rxdw1 & R92S_RXDW1_MOREFRAG) != 0 &&
	    MS(rxdw2, R92S_RXDW2_FRAG) == 0) {
		DPRINTF(0, (CE_CONT, "!%s: %s: morefrag:%d fragnum:%d",
		    dp->name, __func__,
		    (rxdw1 & R92S_RXDW1_MOREFRAG) != 0,
		    MS(rxdw2, R92S_RXDW2_FRAG)));
	}
...
rsu_rx_multi_frame(struct uwgem_dev *dp, mblk_t *mp_head)
{
	...
	pktlen = MS(letoh32(stat->rxdw0), R92S_RXDW0_PKTLEN);
	DPRINTF(10, (CE_CONT, "!%s: %s: Rx %d frames in one chunk(%d bytes)",
	    dp->name, __func__, npkts, pktlen));

	/* Process all of them. */
	reuse_mp = mp_head;
	while (npkts-- > 0) {
		if (len < sizeof (*stat)) {
			cmn_err(CE_CONT, "!%s: %s: Rx corrupted",
			    dp->name, __func__);
			break;
		}
		stat = (struct r92s_rx_stat *)buf;
		...
		if (totlen > len) {
			cmn_err(CE_CONT, "!%s: %s: Rx too short pkt",
			    dp->name, __func__);
			break;
		}
...
#define	RSU_MAXBUFSZ	(3*1024)	/* from 2.6.6.0 2012/04/05 */
...
#endif
		error = uwgem_bulk_out(dp, mp, pipe, 0 /* async: 1 */, 5, 3);
		if (error != USB_SUCCESS) {
			break;
		}
		buf += mlen;
...
rsu_load_firmware(struct uwgem_dev *dp)
{
	...
	DPRINTF(0, (CE_CONT, "!%s: imemsz:%d(0x%x), ememsz:%d(0x%x), dmemsz:%d",
...
rsu_init_chip(struct uwgem_dev *dp)
{
	...
	if (!lp->initialized) {
		...
		/* Load firmware. */
		DPRINTF(10, (CE_CONT, "!%s: %s: before dl_fw: SYS_FUNC_EN:%x",
...
#endif
		DPRINTF(10, (CE_CONT, "!%s: %s: after dl_fw: SYS_FUNC_EN:%x",
		    dp->name, __func__, rsu_read_1(dp, R92S_SYS_FUNC_EN + 1)));
		...
		rsu_write_1(dp, 0xfe5b, 0x4);	/* was 4 */
	...
	error = rsu_fw_iocmd(dp, mode);
	if (error != USB_SUCCESS) {
		cmn_err(CE_WARN,
		    "!%s: %s: could not set video mode %x",
		    dp->name, __func__, mode);
		goto fail;
	}
...
rsu_attach_chip(struct uwgem_dev *dp)
{
	...
	ieee80211_debug &= ~(IEEE80211_MSG_NODE);
	ieee80211_debug &= ~(IEEE80211_MSG_SCAN);
	ieee80211_debug &= ~(IEEE80211_MSG_STATE);
#if 0
	ieee80211_debug |= IEEE80211_MSG_HT;
#endif
...
rsudetach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	if (cmd == DDI_DETACH) {
		/* Power off adapter. */
		rsu_power_off(UWGEM_GET_DEV(dip));
