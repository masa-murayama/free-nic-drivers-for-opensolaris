...
(2) Add a hostname for the NIC into /etc/hosts file
...
(4) Making binaries (only for people want to compile the driver by yourself)
To make the binaries, you need Sun C compiler or gcc version 3. Do
the following operation.

	% /usr/ccs/bin/make

(5) Making binaries for Crossbow
The driver also works with GLD v3, the latest Crossbow network framework
in OpenIndiana and/or Solaris11.
...
(1) copy the nfo driver into the kernel directory
    ...
    If you haven't tested the nfo driver yet, execute the following commands:
            # ./adddrv.sh
	    # init 6	(reboot)
            # devfsadm -i nfo (for solaris7, use drvconfig and reboot with -r)
